...
(function () {
  ...
  function initSchedulePage() {
    ...
    // 跨天或跨周时重新请求，让“今天”高亮和默认教学周保持最新。
    var dateMarker = document.getElementById('scheduleDateMarker');
    if (dateMarker) {
      var shownDate = dateMarker.dataset.date;
      window.setInterval(function () {
        var now = new Date();
        var today = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0');
        if (today !== shownDate) window.location.reload();
      }, 60000);
    }
  }
