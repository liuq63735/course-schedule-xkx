...
def register():
    if request.method == "POST":
        ...
        execute(
            "INSERT INTO users(student_id, password_hash, name, identity, department, major, cohort, role, created_at)"
            " VALUES (?,?,?,?,?,?,?,?,?)",
            (student_id, generate_password_hash(password), name, identity, department, major,
...
def admin_users():
    users = query(
        ...
        "FROM users u ORDER BY u.role = 'admin' DESC, "
        "CASE u.department "
...
def admin_user_edit(uid):
    ...
    if request.method == "POST":
        ...
        if err:
            ...
            if identity == "主席":
                department = None
            execute("UPDATE users SET name=?, identity=?, department=?, major=?, cohort=? WHERE id=?",
                    (name, identity, department, major, cohort, uid))
