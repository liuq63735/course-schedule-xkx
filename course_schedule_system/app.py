...
def init_db():
    ...
    if "major" not in columns:
        db.execute("ALTER TABLE users ADD COLUMN major TEXT NOT NULL DEFAULT ''")
    if "gender" not in columns:
        db.execute("ALTER TABLE users ADD COLUMN gender TEXT NOT NULL DEFAULT '男'")
...
def register():
    if request.method == "POST":
        ...
        execute(
            "INSERT INTO users(student_id, password_hash, name, identity, department, major, gender, phone, cohort, role, created_at)"
            " VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (student_id, generate_password_hash(password), name, identity, department, major, gender, phone,
...
def settings():
    ...
    if request.method == "POST":
        ...
        if not major:
            flash("专业不能为空", "danger")
        elif any((old, new, confirm)) and not check_password_hash(user["password_hash"], old):
            flash("原密码错误", "danger")
        elif any((old, new, confirm)) and len(new) < 6:
            flash("新密码长度至少为 6 位", "danger")
        elif any((old, new, confirm)) and new != confirm:
            flash("两次输入的新密码不一致", "danger")
        else:
            if any((old, new, confirm)):
                execute("UPDATE users SET major = ?, password_hash = ? WHERE id = ?",
                        (major, generate_password_hash(new), user["id"]))
            else:
                execute("UPDATE users SET major = ? WHERE id = ?", (major, user["id"]))
            flash("设置已保存", "success")
            return redirect(url_for("schedule"))
...
def schedule():
    ...
    member_search = (request.args.get("member_search") or "").strip()
    if member_search:
        like = f"%{member_search}%"
        members = query(
            "SELECT * FROM users WHERE name LIKE ? OR student_id LIKE ? OR major LIKE ? "
            "ORDER BY name ASC, student_id ASC",
            (like, like, like),
        )
    else:
        members = query("SELECT * FROM users ORDER BY name ASC, student_id ASC")
...
def admin_users():
    ...
    if department_filter != "all":
        if department_filter == "无部门":
            user_sql += " WHERE u.department IS NULL"
        else:
            user_sql += " WHERE u.department = ?"
            user_args.append(department_filter)
    user_sql += " ORDER BY u.role = 'admin' DESC, "
    user_sql += (
        "CASE u.department "
        "WHEN '办公室' THEN 1 WHEN '科普部' THEN 2 WHEN '学术部' THEN 3 "
        "WHEN '宣传部' THEN 4 WHEN '外联部' THEN 5 WHEN '网络部' THEN 6 "
        "ELSE 7 END ASC, u.name ASC, u.cohort DESC"
    )
    users = query(user_sql, user_args)
    department_counts = query(
        "SELECT COALESCE(department, '无部门') AS department, COUNT(*) AS count "
        "FROM users GROUP BY department ORDER BY CASE department "
        "WHEN '办公室' THEN 1 WHEN '科普部' THEN 2 WHEN '学术部' THEN 3 "
        "WHEN '宣传部' THEN 4 WHEN '外联部' THEN 5 WHEN '网络部' THEN 6 ELSE 7 END")
...
def admin_user_edit(uid):
    ...
    if request.method == "POST":
        ...
        if err:
            ...
            if identity == "主席":
                department = None
            execute("UPDATE users SET name=?, identity=?, department=?, major=?, gender=?, phone=?, cohort=? WHERE id=?",
                (name, identity, department, major, gender, phone, cohort, uid))
